...
namespace AuxiliaryLibraries
{
    ...
    public class AuxiliaryIntPriceModel : AuxiliaryBasicPriceModel<int>
    {
        ...
        public AuxiliaryIntPriceModel(int price, Currency priceBaseCurrency = Currency.IRR, Currency priceTargetCurrency = Currency.Toman, bool metricSystem = false)
        {
            try
            {
        ...
        /// <param name="priceDescriptyion"></param>
        /// <param name="metricSystem"></param>
        protected override void CalculateDecimals(int price, int power, ref int priceShortFormat, ref string priceCurrency, ref string priceDescriptyion, bool metricSystem)
        {
            double remained = Convert.ToDouble(price.ToString().Replace("0", string.Empty).Replace(".", string.Empty));
            priceCurrency = GetPriceCurrency(power, metricSystem);
            var result = Calculate((int)remained);
